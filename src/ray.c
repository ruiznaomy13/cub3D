...
void	ft_raytracing(t_game *game)
{
	...
	while (i < SCR_W)
	{
		...
		game->ray_cast->deltaDistX = (game->ray_cast->rayDirX == 0) ? 1 : fabs(1 / game->ray_cast->rayDirX);
		game->ray_cast->deltaDistY = (game->ray_cast->rayDirY == 0) ? 1 : fabs(1 / game->ray_cast->rayDirY);
